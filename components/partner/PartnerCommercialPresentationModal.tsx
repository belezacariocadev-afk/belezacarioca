...
import { downloadCommercialPresentationPdf } from '@/lib/partner/commercialPresentationPdf';
import { PartnerCommercialPresentationPrint } from './PartnerCommercialPresentationPrint';

type PartnerCommercialPresentationModalProps = {
  open: boolean;
  onClose: () => void;
};
...
export function PartnerCommercialPresentationModal({
  open,
  onClose,
}: PartnerCommercialPresentationModalProps) {
  ...
  const counterLabel = useMemo(
    () => `${activeIndex + 1} de ${commercialPresentationSlides.length}`,
    [activeIndex],
  );
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await downloadCommercialPresentationPdf({
        rootElement: printContentRef.current,
        fileName: 'apresentacao-comercial-beleza-carioca.pdf',
      });
    } catch (error) {
      console.error('Erro ao gerar PDF da apresentacao comercial:', error);
  ...
  function renderSlideContent() {
    ...
    if (activeSlide.id === 'sobre') {
      return (
        <div className="grid h-full gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="flex flex-col">
            <h3 className="text-[clamp(1.28rem,3.2vw,2.35rem)] font-black leading-[1.08] tracking-[-0.04em] text-[color:var(--bc-text)]">
              {activeSlide.title}
            </h3>
    ...
    if (activeSlide.id === 'entregas') {
      return (
        <div className="flex h-full flex-col">
          <h3 className="text-[clamp(1.2rem,3vw,2.1rem)] font-black leading-[1.08] tracking-[-0.04em] text-[color:var(--bc-text)]">
            {activeSlide.title}
          </h3>
          <p className="mt-2 max-w-[760px] text-sm leading-7 text-[color:var(--bc-muted)] sm:text-base">
            {activeSlide.body}
  ...
  return (
    <div className="fixed inset-0 z-[86]">
      <button
        ...
        aria-label="Fechar apresentacao comercial"
      />
          ...
          <header className="border-b border-[rgba(120,84,162,0.12)] px-5 py-4 sm:px-7 sm:py-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="max-w-3xl">
                <p className="bc-kicker">Preview institucional</p>
